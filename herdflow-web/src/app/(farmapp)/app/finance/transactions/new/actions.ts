...
export async function addTransaction(
  ...
  if (type !== "income" && type !== "expense") return { error: "Select income or expense." };
  ...
  if (isRecurring && !RECURRENCES.includes(recurrence as (typeof RECURRENCES)[number])) {
    return { error: "Select how often this transaction recurs." };
  }

  const vatAmount = vatOn ? Math.round(amount * 0.15 * 100) / 100 : 0;
