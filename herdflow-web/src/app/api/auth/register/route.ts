...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (isMobileRegistration) {
      ...
      if (!isFarmer) {
        const suppliedCode =
          (b.farmCode as string | undefined)?.trim().toUpperCase() ??
          (b.inviteCode as string | undefined)?.trim().toUpperCase();
        if (!suppliedCode) {
          return NextResponse.json(
            {
              error:
                "A farm invite code is required. Ask your farm owner to generate one from their HerdFlow Profile screen.",
        ...
        if (
          !suppliedCode.startsWith("HF-") &&
          suppliedCode.includes("-") &&
          suppliedCode.length === 9
        ) {
          const invite = await prisma.farmInvite.findUnique({
            where: { inviteCode: suppliedCode },
          });
          if (!invite || invite.status !== "PENDING") {
            return NextResponse.json(
              { error: "Invite code not found or already used.", code: "FARM_CODE_INVALID" },
              { status: 400 },
            );
          }
          if (new Date(invite.expiresAt) < new Date()) {
            return NextResponse.json(
              {
                error: "This invite code has expired. Ask your farm owner for a new one.",
                code: "INVITE_EXPIRED",
              },
              { status: 410 },
            );
          ...
          ownerProfile = await prisma.farmerProfile.findUnique({
            where: { userId: invite.farmOwnerId },
          });
          inviteId = invite.id;
        } else {
      ...
      if (!farmCode) {
        return NextResponse.json(
          { error: "Could not generate farm code. Please try again." },
          { status: 500 },
        );
      }
