...
export async function POST(request: Request) {
  ...
  if (!role || !["FARM_MANAGER", "FARM_WORKER"].includes(role)) {
    return NextResponse.json({ error: "role must be FARM_MANAGER or FARM_WORKER" }, { status: 400 });
  }
  ...
  if (!profile || !profile.farmCode) {
    return NextResponse.json({ error: "Only farm owners can generate invite codes" }, { status: 403 });
  }
  ...
  if (!inviteCode) return NextResponse.json({ error: "Could not generate code, please try again" }, { status: 500 });
