...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  try {
    const lot = await prisma.auctionLot.findUnique({ where: { id: lotId }, include: { _count: { select: { bids: true } } } });
  ...
  try {
    const lot = await prisma.auctionLot.update({
      ...
      data: {
        ...
        ...(body.healthStatus !== undefined && { healthStatus: String(body.healthStatus).trim() || null }),
        ...(body.images !== undefined && { images: (body.images as string[]).filter(Boolean) }),
        ...(body.documents !== undefined && { documents: (body.documents as string[]).filter(Boolean) }),
        ...(body.startingPriceCents !== undefined && { startingPriceCents: Math.round(Number(body.startingPriceCents)) }),
        ...(body.reservePriceCents !== undefined && { reservePriceCents: body.reservePriceCents ? Math.round(Number(body.reservePriceCents)) : null }),
        ...(body.status !== undefined && { status: body.status as "PENDING" | "OPEN" | "SOLD" | "PASSED" | "CANCELLED" }),
      },
    });
    return NextResponse.json({ lot });
  } catch (err) {
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  } catch { return NextResponse.json({ error: "Failed to delete lot" }, { status: 500 }); }
}
