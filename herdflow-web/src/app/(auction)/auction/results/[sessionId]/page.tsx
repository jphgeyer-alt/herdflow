  ...
  return (
    <div className="min-h-screen bg-[#f5f4ef] px-4 py-10">
      <div className="mx-auto max-w-5xl space-y-8">
        <div>
          <Link href="/auction/results" className="text-sm text-[#2E7D32] hover:underline">
            ← All Results
          </Link>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
          {[
            ...
            {
              label: "Avg Price",
              value: sold.length > 0 ? zar(avgPrice) : "—",
              color: "text-[#5d7497]",
            },
          ].map((s) => (
            <div
              key={s.label}
              className="rounded-xl border border-[#e4ebf5] bg-white p-4 text-center shadow-sm"
            >
        ...
        {/* Results table */}
        {results.length === 0 ? (
          <div className="rounded-2xl border border-[#e4ebf5] bg-white p-8 text-center text-[#5d7497]">
            No results recorded for this auction.
          </div>
        ) : (
          <div className="overflow-hidden rounded-2xl border border-[#e4ebf5] bg-white shadow-sm">
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-[#e4ebf5] bg-[#f5f8fd] text-xs font-semibold uppercase tracking-wide text-[#5d7497]">
                  <tr>
                    ...
                    <th className="hidden px-4 py-3 text-left sm:table-cell">Start Price</th>
                    <th className="px-4 py-3 text-right">Winning Bid</th>
                    <th className="hidden px-4 py-3 text-left md:table-cell">Bids</th>
                ...
                <tbody className="divide-y divide-[#f0f4fb]">
                  {results.map((r) => (
                    <tr key={r.id} className="transition hover:bg-[#f8fafd]">
                      ...
                      <td className="hidden px-4 py-3 text-[#5d7497] sm:table-cell">
                        {zar(r.startPrice)}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-[#2E7D32]">
                        {r.winningBid ? zar(r.winningBid) : "—"}
                      </td>
                      <td className="hidden px-4 py-3 text-[#9aabb9] md:table-cell">
                        {r.totalBids}
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${LOT_STATUS_STYLE[r.lotStatus] ?? "bg-gray-100 text-gray-600"}`}
                        >
                          {r.lotStatus}
                        </span>
