...
export function LiveAuctionRoom({ auctionId, auctionTitle, auctionStatus, initialLots }: Props) {
  ...
  const openBidForm = useCallback((lot: LotSnapshot) => {
    ...
    <div className="space-y-4">
      ...
      {isLive && (
        <div className="flex items-center gap-2 text-xs font-medium">
          <span
            className={`inline-block h-2 w-2 rounded-full ${connected ? "animate-pulse bg-green-500" : "bg-red-400"}`}
          />
          {connected
            ? "Live feed connected — bids update automatically"
      ...
      {bidForm && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 sm:items-center">
          <form
            onSubmit={placeBid}
            className="w-full max-w-md space-y-4 rounded-t-2xl bg-white p-6 shadow-xl sm:rounded-2xl"
          >
            <div>
              <h2 className="text-brand-navy text-lg font-semibold">Place Bid</h2>
            ...
            <div className="flex gap-3">
              <button
                type="submit"
                disabled={bidding}
                className="bg-brand-navy flex-1 rounded-lg py-2.5 text-sm font-semibold text-white disabled:opacity-50"
              >
      ...
      {lots.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {lots.map((lot) => (
            <article
              key={lot.id}
              className="flex flex-col rounded-xl border border-[#d8e0ec] bg-white shadow-sm"
            >
              <div className="flex-1 space-y-2 p-4">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <h3 className="text-brand-navy font-semibold">{lot.title}</h3>
                  </div>
                  <span
                    className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-semibold ${LOT_STATUS_BADGE[lot.status]}`}
                  >
                    {lot.status}
                  </span>
                </div>
                ...
                {lot.status === "SOLD" && lot.winnerName && (
                  <p className="text-xs font-medium text-green-700">Sold to {lot.winnerName}</p>
                )}
              </div>
              ...
              {isLive && lot.status === "OPEN" && (
                <div className="border-t border-[#e4ebf5] px-4 py-3">
                  <button
                    ...
                    className="bg-brand-navy w-full rounded-lg py-2 text-sm font-semibold text-white"
                  >
                    Place Bid
                  </button>
                </div>
