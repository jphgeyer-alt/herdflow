...
app.post('/api/vaccines', (req, res) => {
  ...
  const hasTarget = payload.campId !== undefined && payload.campId !== null ? true : payload.cattleId !== undefined && payload.cattleId !== null;
  if (!hasTarget || !treatmentName || !dueDate) {
    return res.status(400).json({ error: 'Camp or cattle, medicine name, and due date are required.' });
  }
...
app.get('/api/admin/marketplace/items', (req, res) => {
  if (!isAdminAuthorized(req)) {
    return res.status(401).json({ error: 'Admin authorization required for draft catalog access.' });
  }
...
app.post('/api/marketplace/items', (req, res) => {
  const payload = req.body as { name?: string; price?: string; unit?: string; description?: string; imageUrl?: string; stock?: number; isPublished?: boolean };
...
app.post('/api/analytics/events', (req, res) => {
  ...
  const now = Date.now();
  const recent = (analyticsRequestLog.get(requestIp) || []).filter((stamp) => now - stamp < analyticsRateWindowMs);
  if (recent.length >= analyticsRateLimit) {
    return res.status(429).json({ error: 'Too many analytics events from this source. Please retry shortly.' });
  }
  recent.push(now);
  ...
  const created = createAnalyticsEvent({
    ...
    listPosition: Number.isFinite(Number(payload.listPosition)) ? Number(payload.listPosition) : undefined,
...
app.post('/api/orders', (req, res) => {
  ...
  if (!payload.customerName || !payload.customerEmail || !payload.customerPhone || !payload.deliveryAddress) {
    return res.status(400).json({ error: 'Customer name, email, phone, and delivery address are required.' });
  }
  ...
  const cleanLines = payload.lines
    .filter((line) => line.itemId && line.name && line.price && line.unit && line.quantity && line.quantity > 0)
    .map((line) => ({
  ...
  for (const line of cleanLines) {
    ...
    const stock = sourceItem && Number.isFinite(Number(sourceItem.stock)) ? Number(sourceItem.stock) : 0;
  ...
  const created = createMarketplaceOrder({
    ...
    paymentStatus: payload.paymentMethod && payload.paymentMethod !== 'PayOnDelivery' ? 'Initiated' : 'Pending',
    paymentReference: '',
    lines: cleanLines,
...
app.get('/api/orders/track', (req, res) => {
  ...
  const found = getAllMarketplaceOrders().find((entry) =>
...
app.post('/api/marketplace/registrations', (req, res) => {
  ...
  if (!payload.certificationType || !payload.name || !payload.companyName || !payload.phone || !payload.email || !payload.region) {
    return res.status(400).json({ error: 'Certification type, name, company name, phone, email, and region are required.' });
  }
