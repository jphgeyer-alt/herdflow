...
const DEFAULT_SITE_URL =
  typeof window !== "undefined" ? window.location.origin : "https://herdflow-h619.onrender.com";
const DEFAULT_MARKETPLACE_URL = `${DEFAULT_SITE_URL}${MARKETPLACE_PATH}`;
const DEFAULT_APP_DOWNLOAD_URL = "https://expo.io/builds";
const SITE_URL =
  (import.meta as ImportMeta & { env: { VITE_SITE_URL?: string } }).env.VITE_SITE_URL?.trim() ||
  DEFAULT_SITE_URL;
const MARKETPLACE_URL =
  (
    import.meta as ImportMeta & { env: { VITE_MARKETPLACE_URL?: string } }
  ).env.VITE_MARKETPLACE_URL?.trim() || DEFAULT_MARKETPLACE_URL;
const ADMIN_API_KEY =
  (
    import.meta as ImportMeta & { env: { VITE_ADMIN_API_KEY?: string } }
  ).env.VITE_ADMIN_API_KEY?.trim() || "";
const APP_DOWNLOAD_URL =
  (
    import.meta as ImportMeta & {
      env: { VITE_APP_DOWNLOAD_URL?: string; VITE_APK_DOWNLOAD_URL?: string };
    }
  ).env.VITE_APP_DOWNLOAD_URL?.trim() ||
  (
    import.meta as ImportMeta & {
      env: { VITE_APP_DOWNLOAD_URL?: string; VITE_APK_DOWNLOAD_URL?: string };
    }
...
const initialRegistrationForm = {
  certificationType: "Logistics Certified Client" as
    "Logistics Certified Client" | "Certified Livestock Seller",
...
const businessHighlights = [
  {
    ...
    description:
      "Package HerdFlow as an Android app and share it with staff, partners, and buyers.",
  },
  {
...
const websiteSteps = [
  {
    ...
    description:
      "Open HerdFlow in a browser to present the business, the marketplace, and your services.",
  },
  {
  ...
  },
  {
    ...
    description:
      "Collect registrations from logistics companies and farmers who want to work with HerdFlow.",
  },
];
...
const globalCommercePillars = [
  {
    ...
    detail:
      "Operational workflows support regional logistics partners, international buyer onboarding, and centralized order governance.",
  },
  {
    title: "Commercial reliability",
    detail:
      "Inventory is synced with operations, checkout captures buyer data cleanly, and tracking stays transparent across order stages.",
  },
  {
    title: "Enterprise support model",
    detail:
      "Business teams can coordinate procurement, delivery updates, and marketplace admin actions from one shared platform.",
  },
];
...
function getExperimentContext() {
  ...
  const experiment =
    ...
    localStorage.getItem("herdflow:exp") ||
    "default";
...
function App() {
  ...
  const [marketplaceRegistrations, setMarketplaceRegistrations] = useState<
  ...
  const [shopSort, setShopSort] = useState<"featured" | "price-low" | "price-high" | "name">(
    "featured",
  );
  ...
  const [analyticsTransportHistory, setAnalyticsTransportHistory] = useState<
    AnalyticsTransportAttempt[]
  >([]);
  const [releaseReadinessHistory, setReleaseReadinessHistory] = useState<
  ...
  const storefrontItems = useMemo(() => {
    ...
    const filtered = customerVisibleItems.filter((item) => {
      const matchesCategory =
        shopCategory === "All" || classifyMarketplaceCategory(item) === shopCategory;
  ...
  const buildAnalyticsSummary = (events: CommerceAnalyticsEvent[]) => {
    ...
    const sourceBreakdown = new Map<
      string,
      { checkoutClicks: number; orderAttempts: number; orderSuccesses: number }
    >();
    ...
    events.forEach((entry) => {
      ...
      if (typeof entry.source === "string" && entry.source.trim()) {
        ...
        const current = sourceBreakdown.get(source) || {
          checkoutClicks: 0,
          orderAttempts: 0,
          orderSuccesses: 0,
        };
    ...
    const viewToCartRate =
      counts.product_view > 0 ? (counts.add_to_cart / counts.product_view) * 100 : 0;
    const cartToCheckoutRate =
      counts.add_to_cart > 0 ? (counts.checkout_click / counts.add_to_cart) * 100 : 0;
    const checkoutToOrderRate =
      counts.checkout_click > 0 ? (counts.place_order_success / counts.checkout_click) * 100 : 0;
    ...
    const sourceSegments = [...sourceBreakdown.entries()]
      .map(([source, values]) => ({
        ...
        ...values,
        checkoutToOrderRate:
          values.checkoutClicks > 0 ? (values.orderSuccesses / values.checkoutClicks) * 100 : 0,
      }))
  ...
  const analyticsSummary = useMemo(
    () => buildAnalyticsSummary(filteredAnalyticsEvents),
    [filteredAnalyticsEvents],
  );
  const previousAnalyticsSummary = useMemo(
    () => buildAnalyticsSummary(previousWindowEvents),
  ...
  const analyticsAnomalies = useMemo(() => {
    ...
    checks.forEach((item) => {
      ...
      if (item.current <= item.previous * (1 - item.relativeDrop)) {
        flags.push(
          `${item.label} dropped from ${formatPercent(item.previous)} to ${formatPercent(item.current)}.`,
        );
      }
    });
    ...
    if (
      ...
      analyticsSummary.counts.place_order_success <=
        Math.floor(previousAnalyticsSummary.counts.place_order_success * 0.7)
    ) {
      flags.push(
  ...
  const analyticsTransportErrorRate =
    analyticsTransportTotal > 0 ? (analyticsPostFailureCount / analyticsTransportTotal) * 100 : 0;
  ...
  const releaseReadinessChecks = useMemo(() => {
    ...
    const checks = [
      ...
      },
      {
        ...
        pass:
          apiHealth?.status === "ok" && apiHealthLatencyMs !== null && apiHealthLatencyMs <= 1500,
        detail:
          apiHealthLatencyMs !== null
  ...
  function exportAnalyticsCsv() {
    ...
    analyticsSummary.funnelDropOff.forEach((item) => {
      appendRow([item.stage, item.from, item.to, item.dropOff, formatPercent(item.dropOffRate)]);
    });
  ...
  function loadOperationalHistory() {
    ...
    try {
      ...
      const transport = transportRaw
        ? (JSON.parse(transportRaw) as AnalyticsTransportAttempt[])
        : [];
  ...
  function handleFieldChange<K extends keyof typeof initialCattleForm>(
    key: K,
    value: (typeof initialCattleForm)[K],
  ) {
    setCattleForm((current) => ({ ...current, [key]: value }));
  }

  function handleCampField<K extends keyof typeof initialCampForm>(
    key: K,
    value: (typeof initialCampForm)[K],
  ) {
    setCampForm((current) => ({ ...current, [key]: value }));
  }

  function handleVaccineField<K extends keyof typeof initialVaccineForm>(
    key: K,
    value: (typeof initialVaccineForm)[K],
  ) {
    setVaccineForm((current) => ({ ...current, [key]: value }));
  }

  function handleCountField<K extends keyof typeof initialCountForm>(
    key: K,
    value: (typeof initialCountForm)[K],
  ) {
  ...
  function renderMarketplaceImage(
    name: string,
    imageUrl?: string,
  ...
  const cartDetails = useMemo(() => {
    const lines = cartItems
      ...
      .filter((entry): entry is { item: MarketplaceItem; quantity: number; lineTotal: number } =>
        Boolean(entry),
      );
  ...
  async function trackMarketplaceOrder(event: React.FormEvent) {
    ...
    try {
      const response = await fetch(
        `/api/orders/track?orderNumber=${encodeURIComponent(trackingLookup.orderNumber)}&email=${encodeURIComponent(trackingLookup.email)}`,
      );
      if (!response.ok) {
  ...
  async function updateMarketplaceOrder(
    order: MarketplaceOrder,
    status: MarketplaceOrder["status"],
  ) {
    const updatedLocal = marketplaceOrders.map((entry) =>
  ...
  async function fetchRemoteData() {
    ...
    try {
      ...
      saveLocal(
        ...
        (marketplaceData as MarketplaceItem[]).map(normalizeMarketplaceItem),
      );
      ...
      if (usedPublicMarketplaceFallback) {
        setSyncMessage(
          "Connected: remote data loaded. Draft catalog is hidden until VITE_ADMIN_API_KEY is configured.",
        );
      } else {
  ...
  useEffect(() => {
    ...
    setAnalyticsLastTransportAt(
      analyticsTransportHistory.length > 0
        ? analyticsTransportHistory[analyticsTransportHistory.length - 1].at
        : null,
    );
  }, [analyticsTransportHistory]);
  ...
  useEffect(() => {
    ...
    const next: ReleaseReadinessSnapshot = {
      ...
      scorePercent:
        releaseReadinessChecks.total > 0
  ...
  useEffect(() => {
    ...
    const unseen = storefrontItems
      .filter((item) => !viewedItemIds.current.has(item.id))
      .slice(0, 8);
  ...
  async function saveCamp(event: React.FormEvent) {
    ...
    const updated = editingCampId
      ? camps.map((item) => (item.id === editingCampId ? localCamp : item))
      : [localCamp, ...camps];
    setCamps(updated);
  ...
  async function removeCamp(camp: Camp) {
    ...
    const reassigned = cattle.map((item) =>
      item.campId === camp.id ? { ...item, campId: null } : item,
    );
  ...
  async function saveVaccine(event: React.FormEvent) {
    ...
    const updated = editingVaccineId
      ? vaccines.map((item) => (item.id === editingVaccineId ? localVaccine : item))
      : [localVaccine, ...vaccines];
    setVaccines(updated);
  ...
  async function saveMarketplaceItem(event: React.FormEvent) {
    ...
    if (
      !marketplaceForm.name ||
      !marketplaceForm.price ||
      !marketplaceForm.unit ||
      !marketplaceForm.description
    ) {
    ...
    const localItem: MarketplaceItem = normalizeMarketplaceItem({
      ...
      publishedAt: marketplaceForm.isPublished ? existingItem?.publishedAt || now : null,
      ...marketplaceForm,
    });
    ...
    if (!isOffline) {
      try {
        const url = editingMarketplaceId
          ? `/api/marketplace/items/${editingMarketplaceId}`
  ...
  async function compressMarketplaceImage(file: File) {
    ...
    if (width > MARKETPLACE_IMAGE_MAX_DIMENSION || height > MARKETPLACE_IMAGE_MAX_DIMENSION) {
      const ratio = Math.min(
        MARKETPLACE_IMAGE_MAX_DIMENSION / width,
        MARKETPLACE_IMAGE_MAX_DIMENSION / height,
      );
  ...
  async function updateMarketplaceRegistrationStatus(
    entry: MarketplaceRegistration,
    status: MarketplaceRegistration["status"],
  ) {
    const updatedEntry = { ...entry, status };
    const updated = marketplaceRegistrations.map((item) =>
    ...
    if (!isOffline) {
      fetch("/api/customer-signups", {
        ...
        .then((created: CustomerSignup) => {
          setCustomerSignups((current) =>
            current.map((item) => (item.id === signup.id ? created : item)),
          );
          saveLocal("herdflow:customer-signups", [
            created,
            ...customerSignups.filter((item) => item.id !== signup.id),
          ]);
          ...
          <div className="panel">
            ...
            <form className="record-form" onSubmit={trackMarketplaceOrder}>
              <label>
                Confirmation Number
                <input
                  value={trackingLookup.orderNumber}
                  onChange={(event) =>
                    setTrackingLookup((current) => ({
                      ...current,
                      orderNumber: event.target.value,
                    }))
                  }
                  placeholder="HF-20260627-1234"
                />
              </label>
              <label>
                Order Email
                <input
                  value={trackingLookup.email}
                  onChange={(event) =>
                    setTrackingLookup((current) => ({ ...current, email: event.target.value }))
                  }
            ...
            {trackingResult ? (
              <div className="marketplace-grid">
                <article>
                  ...
                  <p className="muted">
                    Payment: {trackingResult.paymentMethod} · {trackingResult.paymentStatus}
                  </p>
                  <p className="muted">Delivery: {trackingResult.deliveryAddress}</p>
                  <p className="muted">
                    Items:{" "}
                    {trackingResult.lines
      ...
      <div className="app-shell storefront-shell">
        <header className="storefront-header">
          <div className="storefront-brand-block">
            ...
            <div>
              ...
              <p>
                Buy feed, health products, and ranch essentials through a premium B2B storefront
                built for fast repeat ordering.
              </p>
            </div>
          </div>
          <div className="storefront-top-actions">
            <a
              className="hero-link"
              href="#checkout"
              onClick={() => trackCheckoutClick("header_link")}
            >
              Go to checkout
            </a>
            <a className="hero-link" href={TRACKING_PATH}>
              Track order
            </a>
            <span className={isOffline ? "storefront-status offline" : "storefront-status online"}>
              {isOffline ? "Offline mode" : "Live inventory"}
            </span>
          </div>
        </header>
        ...
        <section className="storefront-executive-row">
          <article className="storefront-executive-card">
            ...
            <h2>
              Built for serious procurement teams, regional distributors, and growth-focused
              livestock enterprises.
            </h2>
            <p>
              HerdFlow Marketplace is structured for business continuity. Your catalog, stock logic,
              and order pipeline are designed to support local and cross-border commerce with clear
              operational control.
            </p>
          </article>
        ...
        <section className="storefront-hero">
          <div>
            ...
            <p>
              Browse the full catalog, compare pricing, add items to cart, and place orders in
              minutes. Inventory and order confirmations are synced with HerdFlow automatically.
            </p>
            <p className="storefront-region-note">
              Regional checkout preview: {userLocale} · {userCurrency}
            </p>
            <div className="storefront-category-chips">
              {storefrontCategories.map((category) => (
                <button
                  ...
                  type="button"
                  className={
                    shopCategory === category ? "storefront-chip active" : "storefront-chip"
                  }
        ...
        <section className="storefront-proof-row">
          <article className="storefront-proof-card">
            ...
            <p>
              Inventory can move quickly on staple products. Secure quantities now for uninterrupted
              operations.
            </p>
          </article>
          <article className="storefront-proof-card">
            <strong>
              {mostReorderedProduct ? `${mostReorderedProduct.quantity} units` : "No data yet"}
            </strong>
            <h3>Most reordered product</h3>
            <p>
              {mostReorderedProduct
                ? mostReorderedProduct.name
                : "As orders come in, this area highlights repeat buyer demand."}
            </p>
          </article>
        </section>
        ...
        <section className="storefront-toolbar panel">
          ...
          <label>
            ...
            <select
              value={shopSort}
              onChange={(event) =>
                setShopSort(event.target.value as "featured" | "price-low" | "price-high" | "name")
              }
            >
        ...
        <section className="storefront-layout">
          <div className="storefront-products">
            {storefrontItems.length > 0 ? (
              <div className="storefront-grid">
                {storefrontItems.map((item) => (
                  <article key={item.id} className="storefront-card">
                    {renderMarketplaceImage(
                      item.name,
                      item.imageUrl,
                      "marketplace-image storefront-image",
                    )}
                    <div className="storefront-card-body">
                      <p className="storefront-card-category">
                        {classifyMarketplaceCategory(item)}
                      </p>
                      <h3>{item.name}</h3>
                      ...
                      <p className={getItemStock(item) <= 5 ? "stock-badge low" : "stock-badge in"}>
                        {getItemStock(item) <= 5
                          ? `Low stock: ${getItemStock(item)}`
                          : `In stock: ${getItemStock(item)}`}
                      </p>
                      <div className="storefront-card-cta-row">
                        ...
                        <a
                          href="#checkout"
                          className="storefront-quick-checkout-link"
                          onClick={() => trackCheckoutClick("product_card_link")}
                        >
                          Checkout now
                        </a>
                      </div>
              ...
              <div className="panel">
                ...
                <p className="muted">
                  Try a different search term or check back after new stock is published.
                </p>
              </div>
            )}
          </div>
          ...
          <aside className="panel storefront-checkout" id="checkout">
            ...
            <p className="muted">
              Secure order capture for commercial buyers. Confirmation arrives with your unique
              order number.
            </p>
            {lastOrderNumber && (
              <p className="order-confirmation">Latest confirmation: {lastOrderNumber}</p>
            )}
                  ...
                  <article key={line.item.id} className="cart-item">
                    ...
                    <div className="cart-controls">
                      <button
                        type="button"
                        className="secondary"
                        onClick={() => updateCartItemQuantity(line.item.id, line.quantity - 1)}
                      >
                        -
                      </button>
                      ...
                      <button
                        type="button"
                        className="danger"
                        onClick={() => removeItemFromCart(line.item.id)}
                      >
                        Remove
                      </button>
                    </div>
            ...
            <form className="record-form" onSubmit={submitMarketplaceOrder}>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setCheckoutForm((current) => ({
                      ...current,
                      paymentMethod: event.target.value as "PayOnDelivery" | "Stripe" | "PayFast",
                    }))
                  }
                >
              ...
              <button
                type="submit"
                className="primary"
                onClick={() => trackCheckoutClick("checkout_submit_button")}
              >
                Place order
              </button>
            </form>
            <div className="storefront-checkout-notes">
              ...
              <p className="storefront-locale-hint">
                Checkout total is displayed in {userCurrency} based on your locale ({userLocale}).
              </p>
            </div>
          </aside>
        </section>
        ...
        <a
          href="#checkout"
          className="storefront-mobile-checkout-cta"
          onClick={() => trackCheckoutClick("mobile_sticky_cta")}
        >
          {cartDetails.lines.length > 0
      ...
      <div className="app-shell">
          ...
          <div>
            <div className="header-with-logo">
              ...
              <div>
                <h1>HerdFlow</h1>
                <p>
                  Run a modern livestock business with a website, marketplace, and partner
        ...
        <section className="hero-panel">
          <div className="hero-copy">
            ...
            <h2>
              Present HerdFlow as a polished business website before sending people to the
              marketplace or app.
            </h2>
            <p className="hero-description">
              This website gives your customers a clear place to learn what HerdFlow does, buy items
              from the store, and register as logistics partners or livestock sellers.
            </p>
            <div className="hero-actions">
              <button
                type="button"
                className="primary"
                onClick={() => window.location.assign(OPERATIONS_PATH)}
              >
                Open operations
              </button>
              <button
                type="button"
                className="secondary"
                onClick={() => window.location.assign(MARKETPLACE_PATH)}
              >
                Visit marketplace
              </button>
              <a className="hero-link" href={TRACKING_PATH}>
        ...
        <section className="dashboard-grid">
            ...
            <div className="marketplace-grid">
              {websiteBenefits.map((item) => (
                <article key={item}>
                  <h3>{item}</h3>
                  <p className="muted">
                    Built to help customers, staff, and partners work from the same system.
                  </p>
                </article>
              ))}
            </div>
          ...
          <div className="panel">
            ...
            <p className="muted">
              These are the items currently visible in the HerdFlow marketplace.
            </p>
            <div className="marketplace-grid">
        ...
        <section className="dashboard-grid">
          <div className="panel" id="about">
            ...
            <h2>
              Built for livestock businesses that need a clear website, a working marketplace, and a
              stronger customer journey.
            </h2>
            <p className="muted">
              HerdFlow is designed as the public face of your livestock business. The website
              introduces your company, the marketplace sells products, and the app handles
        ...
        <section className="dashboard-grid">
          ...
          <div className="panel">
            ...
            <p className="muted">
              Point the buttons to your live shop, Android APK, or custom domain when you are ready.
            </p>
            <div className="marketplace-actions">
              <a className="marketplace-link" href={MARKETPLACE_PATH}>
        ...
        <section className="dashboard-grid" id="contact">
          ...
          <div className="panel">
            ...
            <p className="muted">
              Capture leads for buyers, farmers, and business partners from the website.
            </p>
            <form className="record-form" onSubmit={saveCustomerSignup}>
              ...
              <label>
                ...
                <textarea
                  ...
                  onChange={(event) =>
                    setCustomerForm((current) => ({ ...current, interest: event.target.value }))
                  }
                  placeholder="Marketplace products, herd tools, partner registration, or support."
                />
              </label>
              <button type="submit" className="primary">
                Request follow-up
        ...
        <section className="dashboard-grid">
          ...
          <div className="panel footer-panel">
            ...
            <h2>HerdFlow</h2>
            <p className="muted">
              A professional livestock business website with marketplace buying, partner
              registrations, desktop access, and Android support.
            </p>
    ...
    <div className="app-shell">
        ...
        <div>
          <div className="header-with-logo">
            ...
            <div>
              <h1>HerdFlow</h1>
              <p>
                Manage livestock operations, marketplace sales, and partner onboarding from one
                business platform.
              </p>
            </div>
          </div>
      ...
      <section className="hero-panel">
        <div className="hero-copy">
          ...
          <h2>
            Run your herd, sell inventory, and onboard partners with a polished customer-ready
            experience.
          </h2>
          <p className="hero-description">
            HerdFlow combines livestock tracking, a marketplace for products and services, and
            registration flows for logistics companies and farmers who want to sell on the app.
          </p>
          <div className="hero-actions">
            <button
              type="button"
            ...
            {APP_DOWNLOAD_URL ? (
              ...
              <span className="hero-note">
                Set VITE_APP_DOWNLOAD_URL or VITE_APK_DOWNLOAD_URL to publish a download link.
              </span>
            )}
          </div>
        </div>
      ...
      <section className="nav-grid">
        {(
          ["dashboard", "cattle", "camps", "vaccines", "counts", "marketplace"] as ViewSection[]
        ).map((item) => (
          <button
            key={item}
        ...
        <>
          ...
          <section className="analytics-admin-grid">
            <div className="panel analytics-admin-panel">
              ...
              <div className="marketplace-actions">
                ...
                <button type="button" className="danger" onClick={() => void clearAnalyticsData()}>
                  Clear events
                </button>
              </div>
              <div
                className="analytics-window-picker"
                role="group"
                aria-label="Analytics date windows"
              >
              ...
              <div className="analytics-rates">
                ...
                <p>
                  <strong>Cart to checkout:</strong>{" "}
                  {formatPercent(analyticsSummary.cartToCheckoutRate)}
                ...
                <p>
                  <strong>Checkout to order:</strong>{" "}
                  {formatPercent(analyticsSummary.checkoutToOrderRate)}
          ...
          <section className="analytics-diagnostics-grid">
            <div className="panel analytics-admin-panel">
              ...
              {analyticsSummary.sourceSegments.length > 0 ? (
                ...
                <p className="muted">
                  No source-level events yet. Track more checkout interactions to populate this
                  segment.
                </p>
              )}
            </div>
              ...
              <div className="analytics-dropoff-list">
                {analyticsSummary.funnelDropOff.map((item) => (
                  <article key={item.stage}>
                    <h3>{item.stage}</h3>
                    <p>
                      From: {item.from} · Next stage: {item.to}
                    </p>
                    <p>
                      Drop-off: {item.dropOff} ({formatPercent(item.dropOffRate)})
                    </p>
                  </article>
                ))}
              </div>
          ...
          <section className="operations-health-grid">
            <div className="panel analytics-admin-panel">
              ...
              <div className="ops-health-cards">
                ...
                <article>
                  ...
                  <p>
                    {analyticsTransportLast24h.failures} failures out of{" "}
                    {analyticsTransportLast24h.total} sends
                  </p>
                </article>
                <article>
                  <h3>Server uptime</h3>
                  <strong>
                    {apiHealth ? `${Math.floor(apiHealth.uptimeSeconds / 60)} min` : "N/A"}
                  </strong>
                  <p>
                    {apiHealth
                      ? `${apiHealth.analyticsEvents} analytics events stored`
                      : "Waiting for /api/health payload."}
                  </p>
                </article>
                <article>
              ...
              <div className="ops-sparkline-grid">
                <article>
                  ...
                  <svg
                    viewBox="0 0 220 52"
                    aria-label="Checkout to order trend sparkline"
                    role="img"
                  >
                ...
                <article>
                  ...
                  <p>
                    Latest:{" "}
                    {formatPercent(
                      errorTrend.length > 0 ? errorTrend[errorTrend.length - 1].value : 0,
                    )}
                  </p>
                </article>
              </div>
            ...
            <div className="panel analytics-admin-panel">
              ...
              <p className="muted">
                {releaseReadinessChecks.passed}/{releaseReadinessChecks.total} checks passed
              </p>
              <div className="release-history-summary">
                <div className="release-history-actions">
                  <button
                    type="button"
                    className="secondary"
                    onClick={exportReleaseReadinessHistoryCsv}
                  >
                    Export Snapshot CSV
                  </button>
                </div>
                ...
                {releaseReadinessDelta ? (
                  <p>
                    Score {formatDelta(releaseReadinessDelta.score)} · Conversion{" "}
                    {formatDelta(releaseReadinessDelta.conversion)} · Error rate{" "}
                    {formatDelta(releaseReadinessDelta.errorRate)}
                  </p>
                ) : (
          ...
          <section className="dashboard-grid">
            <div className="panel">
              ...
              {summary.latestCount ? (
                <div>
                  <p className="muted">
                    {getCampName(summary.latestCount.campId)} ·{" "}
                    {formatDate(summary.latestCount.countDate)}
                  </p>
                  <div className="count-detail">
                    <strong>{summary.latestCount.bulls}</strong> Bulls
                  </div>
                  <div className="count-detail">
                    <strong>{summary.latestCount.cows}</strong> Cows
                  </div>
                  <div className="count-detail">
                    <strong>{summary.latestCount.calves}</strong> Calves
                  </div>
                </div>
              ) : (
            ...
            <div className="panel">
              ...
              {!vaccines.some((v) => !v.givenDate) && (
                <p className="muted">No upcoming vaccines scheduled.</p>
              )}
            </div>
            ...
            <div className="panel">
              <h2>Data Backup</h2>
              <p className="muted">
                Export your herd data to a JSON file or import a previous backup.
              </p>
              <div className="backup-actions">
                <button type="button" className="secondary" onClick={exportBackup}>
                  Export Backup
                </button>
                <button type="button" className="primary" onClick={openBackupPicker}>
                  Import Backup
                </button>
              </div>
              <input
              ...
              <>
                <section className="marketplace-hero">
                  <div>
                    ...
                    <h2>
                      Buy farm essentials, register your business, and join the HerdFlow trade
                      network.
                    </h2>
                    <p className="hero-description">
                      Customers can browse the catalog, while logistics companies and farmers can
                      submit partner requests to sell or move livestock through the platform.
                    </p>
                  </div>
                  <div className="marketplace-hero-actions">
                    {MARKETPLACE_URL ? (
                      ...
                      <p className="hero-note">
                        Set VITE_MARKETPLACE_URL to point buyers to a live store.
                      </p>
                    )}
                    <a className="marketplace-buy" href={TRACKING_PATH}>
                      Track my order
                    </a>
                    <button
                      type="button"
                  ...
                  <div className="panel">
                    <div className="table-header">
                      ...
                      {editingMarketplaceId && (
                        <button type="button" className="secondary" onClick={resetMarketplaceForm}>
                          Cancel
                        </button>
                      )}
                    </div>
                    ...
                    <form className="record-form" onSubmit={saveMarketplaceItem}>
                      ...
                      <label>
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setMarketplaceForm((current) => ({
                              ...current,
                              isPublished: event.target.value === "yes",
                            }))
                          }
                        >
                      ...
                      <label>
                        ...
                        <textarea
                          ...
                          placeholder="Tell buyers what this item is for."
                        />
                      </label>
                      <label>
                        Image URL
                      ...
                      >
                        <p className="muted">
                          Drag and drop a product photo here, or choose a file below.
                        </p>
                        <input
                          type="file"
                          accept="image/*"
                      ...
                      {marketplaceForm.imageUrl && (
                        <>
                          ...
                          <button
                            ...
                            className="secondary"
                            onClick={() =>
                              setMarketplaceForm((current) => ({ ...current, imageUrl: "" }))
                            }
                          >
                            Remove image
                          </button>
                      ...
                      <button type="submit" className="primary">
                        {editingMarketplaceId ? "Save Item" : "Add Item"}
                      </button>
                    </form>
                  </div>
                  <div className="panel">
                    <h2>Store Catalog</h2>
                    <p className="muted">
                      Draft items stay hidden from clients until published. Published items appear
                      on website and mobile storefront views.
                    </p>
                    <label>
                      Catalog Filter
                      <select
                        ...
                        onChange={(event) =>
                          setAdminCatalogFilter(event.target.value as AdminCatalogFilter)
                        }
                      >
                        ...
                        <article key={item.id}>
                          ...
                          <div className="marketplace-actions">
                            ...
                            <button
                              type="button"
                              className="secondary"
                              onClick={() => editMarketplaceItem(item)}
                            >
                              Edit
                            </button>
                            <button
                              type="button"
                              className="danger"
                              onClick={() => removeMarketplaceItem(item)}
                            >
                              Delete
                            </button>
                          </div>
                    ...
                    {adminCatalogItems.length === 0 && (
                      <p className="muted">No items match this filter yet.</p>
                    )}
                    {MARKETPLACE_URL ? (
                      <a
                        className="marketplace-link"
                      ...
                      <p className="muted">
                        Set VITE_MARKETPLACE_URL to connect item buy buttons to a live store.
                      </p>
                    )}
                  </div>
                </section>
                ...
                <section className="dashboard-grid">
                  <div className="panel">
                    <h2>Cart and Checkout</h2>
                    <p className="muted">
                      Build an order from the catalog and submit it for confirmation.
                    </p>
                    {lastOrderNumber && (
                      <p className="order-confirmation">Latest confirmation: {lastOrderNumber}</p>
                    )}
                    {cartDetails.lines.length > 0 ? (
                      <div className="cart-list">
                          ...
                          <article key={line.item.id} className="cart-item">
                            ...
                            <div className="cart-controls">
                              <button
                                ...
                                className="secondary"
                                onClick={() =>
                                  updateCartItemQuantity(line.item.id, line.quantity - 1)
                                }
                              >
                                -
                              </button>
                        ...
                        <p className="cart-total">
                          Estimated total: {formatCurrency(cartDetails.total)}
                        </p>
                      </div>
                    ) : (
                    ...
                    <form className="record-form" onSubmit={submitMarketplaceOrder}>
                      ...
                      <a className="marketplace-link" href={TRACKING_PATH}>
                  ...
                  <div className="panel">
                    ...
                    <p className="muted">
                      Track all website orders and update status for the operations and app teams.
                    </p>
                          ...
                          <article key={order.id}>
                            ...
                            <div className="marketplace-actions">
                              <button
                                type="button"
                                className="secondary"
                                onClick={() => updateMarketplaceOrder(order, "Confirmed")}
                              >
                                Confirm
                              </button>
                              <button
                                type="button"
                                className="secondary"
                                onClick={() => updateMarketplaceOrder(order, "Fulfilled")}
                              >
                                Fulfill
                              </button>
                              <button
                                type="button"
                                className="secondary"
                                onClick={() => updateMarketplaceOrder(order, "Cancelled")}
                              >
                                Cancel
                              </button>
                            </div>
                ...
                <section className="dashboard-grid">
                  <div className="panel">
                    <div className="table-header">
                      ...
                      <button type="button" className="secondary" onClick={resetRegistrationForm}>
                        Reset
                      </button>
                    </div>
                    {error && <p className="error-message">{error}</p>}
                    <form className="record-form" onSubmit={saveMarketplaceRegistration}>
                      <label>
                        Partner Type
                        <select
                          value={registrationForm.certificationType}
                          onChange={(event) =>
                            setRegistrationForm((current) => ({
                              ...current,
                              certificationType: event.target
                                .value as MarketplaceRegistration["certificationType"],
                            }))
                          }
                        >
                          <option value="Logistics Certified Client">Logistics company</option>
                          <option value="Certified Livestock Seller">
                            Farmer / livestock seller
                          </option>
                        </select>
                      </label>
                      <label>
                      ...
                      <label>
                        ...
                        <input
                          ...
                          placeholder="Business, fleet, or ranch name"
                        />
                      </label>
                      <label>
                        Phone
                      ...
                      <label>
                        ...
                        <textarea
                          ...
                          placeholder="Share livestock volume, delivery coverage, or service details."
                        />
                      </label>
                      <button type="submit" className="primary">
                  ...
                  <div className="panel">
                    ...
                    <p className="muted">
                      These partner requests are stored in HerdFlow and can be reviewed later.
                    </p>
                    <div className="marketplace-grid">
                      {marketplaceRegistrations.map((entry) => (
                        <article key={entry.id}>
                          <h3>{entry.name}</h3>
                          <p className="marketplace-price">
                            {getRegistrationTypeLabel(entry.certificationType)}
                          </p>
                    ...
                    {marketplaceRegistrations.length === 0 && (
                      <p className="muted">No registrations submitted yet.</p>
                    )}
                  </div>
                </section>
              </>
        ...
        <div className="content-grid">
          <form className="record-form" onSubmit={saveCattle}>
            <div className="form-header">
              ...
              {editingCattleId && (
                <button type="button" className="secondary" onClick={resetCattleForm}>
                  Cancel
                </button>
              )}
            </div>
            ...
            <label>
              ...
              <textarea
                ...
                placeholder="Health, breeding and location notes"
              />
            </label>

            <button type="submit" className="primary">
              {editingCattleId ? "Save Animal" : "Add Animal"}
            </button>
          </form>
            ...
            <table>
              ...
              <tbody>
                {cattle.map((record) => (
                  <tr key={record.id}>
                    <td>
                      <span className={getColorChipClass(record.colorId)} />
                      {record.tag}
                    </td>
                    <td>{record.breed}</td>
                    ...
                    <td className="actions-cell">
                      <button
                        type="button"
                        className="secondary"
                        onClick={() => editCattle(record)}
                      >
                        Edit
                      </button>
                      <button type="button" className="danger" onClick={() => removeCattle(record)}>
                        Delete
                      </button>
                    </td>
                  </tr>
        ...
        <div className="content-grid">
          <form className="record-form" onSubmit={saveCamp}>
            <div className="form-header">
              ...
              {editingCampId && (
                <button type="button" className="secondary" onClick={resetCampForm}>
                  Cancel
                </button>
              )}
            </div>
            ...
            <label>
              ...
              <input
                value={campForm.name}
                onChange={(event) => handleCampField("name", event.target.value)}
            ...
            <label>
              Color ID
              <select
                value={campForm.colorId}
                onChange={(event) => handleCampField("colorId", event.target.value)}
              >
                {colors.map((color) => (
                  <option key={color} value={color}>
                    {color}
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) => handleCampField("description", event.target.value)}
                placeholder="Location, fences, notes."
              />
            </label>

            <button type="submit" className="primary">
              {editingCampId ? "Save Camp" : "Add Camp"}
            </button>
          </form>

          <section className="table-panel">
            <div className="table-header">
              <h2>Camp Directory</h2>
            </div>
            <table>
              <thead>
                <tr>
              ...
              <tbody>
                {camps.map((camp) => (
                  <tr key={camp.id}>
                    <td>{camp.name}</td>
                    <td>
                      <span className={getColorChipClass(camp.colorId)} />
                      {camp.colorId}
                    </td>
                    <td>{camp.description || "—"}</td>
                    <td className="actions-cell">
                      <button type="button" className="secondary" onClick={() => editCamp(camp)}>
                        Edit
                      </button>
                      <button type="button" className="danger" onClick={() => removeCamp(camp)}>
                        Delete
                      </button>
                    </td>
                  </tr>
        ...
        <div className="content-grid">
          <form className="record-form" onSubmit={saveVaccine}>
            <div className="form-header">
              ...
              {editingVaccineId && (
                <button type="button" className="secondary" onClick={resetVaccineForm}>
                  Cancel
                </button>
              )}
            </div>
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => handleVaccineField("vaccineName", event.target.value)}
            ...
            <div className="field-row">
              <label>
                ...
                <input
                  ...
                  onChange={(event) => handleVaccineField("scheduledDate", event.target.value)}
                />
              </label>
              <label>
                Completed Date
                <input
                  ...
                  onChange={(event) => handleVaccineField("givenDate", event.target.value || null)}
                />
              </label>
            </div>
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) => handleVaccineField("note", event.target.value)}
                placeholder="Dose, provider, or follow-up."
              />
            </label>

            <button type="submit" className="primary">
              {editingVaccineId ? "Save Vaccine" : "Add Vaccine"}
            </button>
          </form>

          <section className="table-panel">
            <div className="table-header">
              <h2>Vaccine & Health Schedule</h2>
            </div>
            <table>
              <thead>
                <tr>
                  ...
                  <tr key={entry.id}>
                    ...
                    <td className="actions-cell">
                      <button
                        type="button"
                        className="secondary"
                        onClick={() => editVaccine(entry)}
                      >
                        Edit
                      </button>
                      <button type="button" className="danger" onClick={() => removeVaccine(entry)}>
                        Delete
                      </button>
                    </td>
                  </tr>
        ...
        <div className="content-grid">
          <form className="record-form" onSubmit={saveCount}>
            ...
            <label>
              Camp
              <select
                value={countForm.campId ?? ""}
                onChange={(event) =>
                  handleCountField("campId", event.target.value ? Number(event.target.value) : null)
                }
              >
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => handleCountField("countDate", event.target.value)}
              />
            </label>
            ...
            <div className="field-row">
              <label>
                ...
                <input
                  type="number"
                  min="0"
                  value={countForm.bulls}
                  onChange={(event) => handleCountField("bulls", Number(event.target.value))}
                />
              </label>
              <label>
                Cows
                <input
                  type="number"
                  min="0"
                  value={countForm.cows}
                  onChange={(event) => handleCountField("cows", Number(event.target.value))}
                />
              </label>
              <label>
                Calves
                <input
                  type="number"
                  min="0"
                  value={countForm.calves}
                  onChange={(event) => handleCountField("calves", Number(event.target.value))}
                />
              </label>
            </div>
            ...
            <label>
              ...
              <textarea
                ...
                onChange={(event) => handleCountField("note", event.target.value)}
                placeholder="Record keeper name or field notes."
              />
            </label>
              ...
              <tbody>
                {counts.map((log) => (
                  <tr key={log.id}>
                    ...
                    <td>{log.calves}</td>
                    <td className="actions-cell">
                      <button type="button" className="danger" onClick={() => removeCount(log)}>
                        Delete
                      </button>
                    </td>
                  </tr>
